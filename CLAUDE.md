...
| `exposure_calibration.py` | The pure maths (numpy only) behind `IdsCamera.auto_calibrate()`: metering modes (`METERING_HIGHLIGHT` for a beam on black, `METERING_FIELD` for the BIO's lit disc — per profile, since a speck pins p99.9), `exposure_budget_us()`, and `next_exposure_gain()`, which prefers exposure up to the budget and the least gain that will do. DECISIONS.md 2026-09-08 and 2026-09-14. |
