...
| `app.py` | The kiosk: a thin PySide6 shell over `KioskController` — the Reflex mark, instrument picker, **Start Recording**, **Stop Recording** (the status line counts up against the session limit), a three-stop **Brightness** slider naming the step it is on (live while recording), and no Watch button: stopping opens the viewer itself (`_review_last_session()`, once per session, skipped if the manifest is missing), because an unexported recording is a lost one. `_with_preview_paused()` stops the preview around the modal while the cameras keep running. The picker disables while recording. Owns no decisions. |
