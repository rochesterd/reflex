...
| File | Role |
|---|---|
| `camera.py` | `Frame` and abstract `BaseCamera` (`set_brightness(amount)`, 0.0–1.0 over a per-model *measured* range, not a hardware limit; `BRIGHTNESS_ADJUSTABLE` says whether there is anything to offer) (capture thread, bounded queue, latest-frame slot). The per-frame `orientation` (`VALID_ORIENTATIONS`, `apply_orientation()`) is applied in `_run()` before queueing, so every consumer sees the same image. |
