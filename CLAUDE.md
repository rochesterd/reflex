...
| `app.py` | The kiosk: a thin PySide6 shell over `KioskController` — the Reflex mark, instrument picker, **Start Recording**, **Stop Recording** (the status line counts up against the session limit), a continuous **Brightness** slider (live while recording; camera writes throttled to `BRIGHTNESS_WRITE_MS`, leading edge and resting value always applied), and no Watch button: stopping opens the viewer itself (`_review_last_session()`, once per session, skipped if the manifest is missing), because an unexported recording is a lost one. Closing that viewer unexported asks then and there, offering a way back to Export — never at app close, which is past the point of acting. `_with_preview_paused()` stops the preview around the modal while the cameras keep running. The picker disables while recording. Owns no decisions. |
