...
| File | Role |
|---|---|
| `camera.py` | `Frame` and abstract `BaseCamera` (`set_brightness(amount)`, 0.0–1.0 over a per-model *measured* range, not a hardware limit; `BRIGHTNESS_ADJUSTABLE` says whether there is anything to offer). The per-frame `orientation` (`VALID_ORIENTATIONS`, `apply_orientation()`) is applied in `_run()` before queueing, so every consumer sees the same image. |
| `exposure_calibration.py` | The pure maths (numpy only) behind `IdsCamera.auto_calibrate()`: metering modes (`METERING_HIGHLIGHT` for a beam on black, `METERING_FIELD` for the BIO's lit disc — per profile, since a speck pins p99.9), `exposure_budget_us()`, and `next_exposure_gain()`, which prefers exposure up to the budget and the least gain that will do. DECISIONS.md 2026-09-14. |
| `device_presets.py` | The supported-camera registry: `DeviceProfile`/`PROFILES` (what settings.py offers and `config.json`'s `profile` resolves through), plus the `*_for_model()` lookups (orientation, pixel clock, black level, metering, resting gamma, pixel format) reading that same table, so a preset and a picked profile can't disagree. `CUSTOM_PROFILE_ID` is deliberately absent from `PROFILES`: unknown degrades to custom, never to a failed load. Consulted by `IdsCamera._open()`; a `config.json` `orientation` overrides it. The older BIO's (no-op) orientation is defaulted in `net2860_winusb_camera.py` instead, since it has no IDS model string. Where this is headed: ROADMAP.md's device-profiles entry. |
...
| `ids_camera.py` | `IdsCamera` — both IDS GenICam cameras, opened by serial; resolves unset presets from `device_presets` by `ModelName()`. `_to_bgr8()` applies the host tone curve *before* the 8-bit conversion; its numpy view doesn't keep the Image alive. `list_ids_devices()` feeds `settings.py`'s dropdowns. |
...
| `net2860_winusb_camera.py` | `Net2860WinUsbCamera` — the older BIO's KS722OUP (EM2860 bridge), `kind: "net2860_winusb"`. The camera board runs its own AE/AWB, so there is no exposure/gain to offer; the bridge's picture registers are ours and applied at open from `net2860_init.PICTURE_DEFAULTS` (Keeler's values, confirmed 2026-09-13), overridable per camera via `picture=`. `Frame.index` is the camera's own field counter, so its gaps are **real** drops. |
| `net2860_init.py` | `START_WRITES`/`STOP_WRITES` — the EM2860 init sequence captured from Keeler's driver and verified by register readback. `STOP_WRITES` is only the record of where the vendor's session ended; nothing replays it (DECISIONS.md 2026-09-13). Picture values are in `PICTURE_DEFAULTS`. Its docstring says how to re-derive it; the capture is gitignored and irreplaceable. |
...
| `session_buffer.py` | `buffer_root()` (under the user's temp dir), `removable_drives_detailed()` (what Export offers as destinations) and `clear_buffer()`, which `app.py` calls at start and at exit; `packaging/clear_reflex_buffer.ps1` is the logon sweep for the crash case. `clear_buffer()` refuses any path outside temp, so a mis-wired `output_root` can't delete real folders. |
| `session_format.py` | The on-disk vocabulary of a session (`SESSION_FORMAT_VERSION`, role names, `MANIFEST_NAME`). No imports, so readers don't pull in the writer. |
...
| `session_reader.py` | `Session.load()` (`format_version: 2` only) and `SessionPlayer` — per stream, the last frame at or before media time *t*; alignment is by PTS alone. No Qt. |
| `viewer.py` | `ViewerDialog` — play, scrub, layout picker, cancellable Export. `confirm_close` can refuse a close, gated in **both** `done()` and `closeEvent()` (Esc never delivers a QCloseEvent; `closeEvent` releases the decoders). `_choose_destination()` asks which removable drive when more than one is plugged in; `on_export` reports a finished export to the kiosk. Opened modally by `app.py` via `open_session()`; no standalone viewer (DECISIONS.md 2026-09-14). Never modifies a recording. Tears down on `finished`, not `closeEvent`. |
...
| `reflex_style.py` | `apply()` — Fusion style, palette, font and a stylesheet keyed on object names. Called by `app.main()` only; `settings.py` and `preview.py` stay native. |
...
| `preview.py` | Dev tool: two live cameras, layout dropdown, frame-index/skew readout, via `get_latest()`. |
...
| `kiosk.py` | `KioskController` — the state machine (idle/ready/recording/error): the `select_instrument()` lifecycle, preflight (liveness, **freshness**, disk space), stall and freeze detection, and the session time limit (`MAX_SESSION_MINUTES`: a normal stop, and what the disk preflight budgets for). Freshness (`_frame_signature()`) catches a camera that delivers frames but has stopped *seeing*. No Qt. |
| `app.py` | The kiosk: a thin PySide6 shell over `KioskController` — the Reflex mark, instrument picker, **Start Recording**, **Stop Recording** (the status line counts up against the session limit), a continuous **Brightness** slider (live while recording; camera writes throttled to `BRIGHTNESS_WRITE_MS`, leading edge and resting value always applied), and no Watch button: stopping opens the viewer itself (`_review_last_session()`, once per session, skipped if the manifest is missing). Closing that viewer unexported is *refused* until answered (`viewer.py`'s `confirm_close`), so "Save it now" leaves the window where it was. `_with_preview_paused()` stops the preview around the modal while the cameras keep running. The picker disables while recording. Owns no decisions. |
...
| `packaging/*.spec` | PyInstaller specs for `app.exe` and `settings.exe`. See `PACKAGING.md`. |
| `packaging/reflex.iss` | The installer: `app.exe` (Desktop and Start menu), `settings.exe` (Start menu only), the bundled IDS peak installed silently, and the legacy BIO's driver package. `AppId` is pinned to `reflex`; changing it orphans existing installs. |
...
Desktop shortcut), which opens the viewer itself — never point a student
at anything else.
...
- Develop against `SyntheticCamera`; cameras are usually not attached.
  Don't hard-code anything that must be measured.
