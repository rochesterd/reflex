...
| `session_buffer.py` | `buffer_root()` (under the user's temp dir), `removable_drives_detailed()` (what Export offers as destinations) and `clear_buffer()`, which `app.py` calls at start and at exit; `packaging/clear_reflex_buffer.ps1` is the logon sweep for the crash case. Nothing here outlives the app, which is why there is no retention. `clear_buffer()` refuses any path outside temp, so a mis-wired `output_root` can't delete real folders. |
...
| `viewer.py` | `ViewerDialog` (play, scrub, live layout picker, cancellable Export; `confirm_close` can refuse a close, gated in **both** `done()` and `closeEvent()` — Esc never delivers a QCloseEvent, and `closeEvent` releases the decoders — `_choose_destination()` asks which removable drive when more than one is plugged in, since picking by letter could write onto a stranger's stick; `on_export` reports a finished export back to the kiosk). A `QDialog` that `app.py` opens modally via `open_session()`; there is no standalone viewer (DECISIONS.md 2026-09-14). Never modifies a recording. Tears down on `finished`, not `closeEvent`: it holds open PyAV decoders. |
...
| `app.py` | The kiosk: a thin PySide6 shell over `KioskController` — the Reflex mark, instrument picker, **Start Recording**, **Stop Recording** (the status line counts up against the session limit), a continuous **Brightness** slider (live while recording; camera writes throttled to `BRIGHTNESS_WRITE_MS`, leading edge and resting value always applied), and no Watch button: stopping opens the viewer itself (`_review_last_session()`, once per session, skipped if the manifest is missing), because an unexported recording is a lost one. Closing that viewer unexported is *refused* until answered (`viewer.py`'s `confirm_close`), so "Save it now" leaves the window where it was — never at app close, which is past the point of acting. `_with_preview_paused()` stops the preview around the modal while the cameras keep running. The picker disables while recording. Owns no decisions. |
